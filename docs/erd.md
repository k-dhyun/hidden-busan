...
- Figma의 `Content.region`은 별도 구/군 테이블 없이 `places.region` 문자열로 저장한다.
- 미션 완료 기록은 `mission_id`를 저장하고, 장소 정보는 `missions.place_id`를 통해 조회한다.
...
2. 지역명은 `places.region`을 사용한다.
3. 카테고리 필터는 `places.category`를 사용한다.
4. 부산 구/군 지도 선택은 프론트의 SVG 구역 ID와 `places.region` 값을 매칭한다.
5. 장소 마커 위치가 필요하면 `latitude`, `longitude`를 사용한다.
...
2. 미션 완료 시 `mission_completions`에 `user_id`, `mission_id`, `achievement_id`, `photo_url`, `stamp_emoji`, `memo`, `completed_at`을 저장한다.
...
| `places` | `INDEX(region)`, `INDEX(category)`, `INDEX(difficulty)` | 지역/카테고리/난이도 필터 |
...
- 완료 기록의 장소 정보는 `mission_completions.mission_id`에서 `missions.place_id`를 거쳐 조회한다.
...
- 스탬프 이모지를 미션 완료 시점의 결과로 고정하려면 `mission_completions.stamp_emoji`를 저장한다. 업적 이모지 변경을 항상 반영하려면 `achievements.emogi`를 조인해 표시한다.
